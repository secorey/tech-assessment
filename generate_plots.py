...
def plot_topics(model, feature_names, n_top_words, file_name, bad_words):
    ...
    for topic_idx, ax in enumerate(axes):
        ...
        colors = ['#0674cc' if word not in bad_words else 'red' for word in top_words]
...
def main():
    ...
    # Get feature names:
    tfidf_feature_names_top = top_5_model.named_steps['tfidf'].get_feature_names_out()
    tfidf_feature_names_bottom = bottom_5_model.named_steps['tfidf'].get_feature_names_out()
